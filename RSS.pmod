...
Channel parse(string xml)
{
  string version;
  Node rss_node;
  Node n = parse_input(xml);

  if(n->get_node_type() != XML_ROOT) error("invalid XML provided!\n");

  // we should look at the children; one of them must be the RSS element
  foreach(n->get_children(); int index; Node child)
  {
    if(child->get_node_type() == XML_ELEMENT && child->get_tag_name() == "rss")
    {
      // eureka! we have found RSS! let's make sure it's a valid version...
      if(child->get_attributes()["version"])
      {
        version=child->get_attributes()["version"];
        rss_node=child;
        break;
      }    
    }

    if(child->get_node_type() == XML_ELEMENT && child->get_tag_name() == "RDF" && child->get_default_ns()== V10_NS)
    {
...
class Thing
{
  ...
  {
    foreach(xml->get_descendants(0); int desc; Node n)
    {
  ...
  static void create(Node xml, string version)
  {
    ...
    foreach(xml->get_children(); int index; Node child)
    {
      if(child->get_node_type() == XML_ELEMENT)
      {
         string e = child->get_tag_name();
         string v;
...
class Channel
{
  ...
  void create(Node xml, string _version)
  {
    ...
    foreach(xml->get_children(); int index; Node child)
    {
      if(child->get_node_type() == XML_ELEMENT && child->get_tag_name() == "channel")
        ::create(child, version);
      if(child->get_node_type() == XML_ELEMENT && child->get_tag_name() == "item")
      {
  ...
  void parse_textinput(Node xml, string version)
  {
    ...
    foreach(xml->get_children(); int index; Node child)
    {
       ...
       if(child->get_node_type() == XML_ELEMENT && required_elements[child->get_tag_name()])
         d[child->get_tag_name()] = get_element_text(xml);
